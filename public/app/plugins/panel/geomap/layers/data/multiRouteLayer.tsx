...
export const MULTI_ROUTE_LAYER_ID = 'multi-route';

// Used by default when nothing is configured
export const defaultRouteConfig: MapLayerOptions<RouteConfig> = {
  type: MULTI_ROUTE_LAYER_ID,
...
type DeviceRouteMap = {
  [key: string]: DeviceRoute;
};

/**
 * Map layer configuration for circle overlay
 */
export const multiRouteLayer: MapLayerRegistryItem<RouteConfig> = {
  id: MULTI_ROUTE_LAYER_ID,
  name: 'Multi Route',
  description: 'Display data points as multiple routes',
  isBaseMap: false,
  showLocation: true,
    ...
    if (!style.fields && !hasArrows) {
      ...
      vectorLayer.setStyle((feature: FeatureLike) => {
        ...
        if (geom instanceof SimpleGeometry) {
          buildDeviceRoutes(geom, feature, options, style).forEach((deviceRoute) => {
            const coordinates = deviceRoute.coordinates;
            if (coordinates) {
              ...
              for (let i = 0; i < coordinates.length - 1; i++) {
                ...
                const color1 = tinycolor(
                  theme.visualization.getColorByName((deviceRoute.color) ?? style.base.color)
                )
                  .setAlpha(opacity)
                  .toString();
                ...
                const arrowSize1 = (deviceRoute.size) ?? style.base.size;
                const arrowSize2 = (deviceRoute.size) ?? style.base.size;
                ...
                const flowStyle = new FlowLine({
                  ...
                  color: color1,
                  color2: color2,
                  width: (deviceRoute.size) ?? style.base.size,
                  width2: (deviceRoute.size) ?? style.base.size,
                });
                if (config.arrow) {
...
function buildDeviceRoutes(
  geometry: any,
  feature: FeatureLike,
  options: any,
  style: StyleConfigState): DeviceRoute[] {
  const dims = style.dims;
  if (!dims) {
    return [];
  }
  const frame = feature.get('frame');
  const fields: any = frame.fields;
  let deviceIds = [];
  if (fields && fields.length > 0) {
    for (let i = 0; i < fields.length - 1; i++) {
      if (fields[i].name === options['trackByDeviceId']) {
        deviceIds = fields[i].values;
      }
    }
  }
  ...
  const map: DeviceRouteMap = {};
  for (let i = 0; i < coordinates.length; i++) {
    const key = deviceIds[i];
    map[key] ??= {
      deviceId: key,
      color: (dims.color && dims.color.get(i)),
      size: (dims.size && dims.size.get(i)),
      coordinates: []
    };
    map[key].coordinates.push(coordinates[i]);
  }
